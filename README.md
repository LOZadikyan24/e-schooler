<h1 align="center"><strong>e-schooler</strong></h1>

<p align="center">
<img alt="e-schooler Logo" src="https://www.mnogoznayko.com/wp-content/uploads/2024/10/graduation.png" width="230px">
</p>

<p align="center">
<strong>e-schooler</strong> is an interactive digital classroom designed to make learning simple, engaging, and effective.  
Explore lessons, practice your skills, and test your knowledge in one place.
</p>

---

## 📑 Table of Contents
- [Description](#-description)
- [Documentation](#-documentation)
- [Features](#-features)
- [Tools and Languages](#%EF%B8%8F-tools-and-languages)
- [Team](#-team)

---

## 📋 Description

<p align="center">
<strong>E-Schooler</strong> is an educational platform that combines learning and testing in a single application.  
It provides structured content, practical exercises, and automatically generated tests based on a categorized question bank.  

💡The goal is to create a more interactive and efficient way for students to learn and evaluate their progress.
</p>

---

## 📖 Documentation

- 📄 **Project Documentation:** `docs/documentation.docx`  
- 📊 **Presentation:** `docs/presentation.pptx`  

---

## 🚀 Features

<p align="center">
• Interactive learning content <br>
• Practice exercises and homework tasks <br>
• Automatic test generation (20 questions) <br>
• Question bank with categories (minimum 30 questions) <br>
• Weighted scoring system <br>
• Automatic grading (Bulgarian 2–6 system) <br>
• Performance statistics and analysis
</p>

---

## 🛠️ Tools and Languages

<p align="center">
<a href="https://visualstudio.microsoft.com/"><img src="https://img.icons8.com/fluency/48/000000/visual-studio.png" alt="Visual Studio"/></a>
<a href="https://www.microsoft.com/en-ww/microsoft-365/word"><img src="https://img.icons8.com/fluency/48/000000/microsoft-word-2019.png" alt="MS Word logo" width=48px /></a>
<a href="https://www.microsoft.com/en-us/microsoft-365/powerpoint"><img src="https://img.icons8.com/fluency/48/000000/microsoft-powerpoint-2019.png" alt="MS PowerPoint logo" width=48px /></a>
<a href="https://github.com"><img src="https://cdn-icons-png.flaticon.com/512/25/25231.png" alt="GitHub" height=48px width=48px/></a>
<a href="https://teams.microsoft.com/"><img src="https://img.icons8.com/?size=100&id=zQ92KI7XjZgR&format=png&color=000000" alt="Teams" width="48px"></a>
<a href="https://www.cplusplus.com/"><img src="https://img.icons8.com/color/48/000000/c-plus-plus-logo.png" alt="C++"/></a>
</p>

---

## 👤 Team

<p align="center">
<table>
  <tr align="center">
    <th><strong>Name</strong></th>
    <th><strong>Role</strong></th>
    <th><strong>Class</strong></th>
  </tr>
  <tr align="center">
    <td><strong>Loris Zadikyan</strong></td>
    <td><strong>Scrum Trainer</strong></td>
    <td>9G</td>
  </tr>
  <tr align="center">
    <td><strong>Bozhidar Stanev</strong></td>
    <td><strong>Front-end Developer</strong></td>
    <td>9B</td>
  </tr>
  <tr align="center">
    <td><strong>Presiyan Tenev</strong></td>
    <td><strong>Back-end Developer</strong></td>
    <td>9G</td>
  </tr>
  <tr align="center">
    <td><strong>Yanko Yanakiev</strong></td>
    <td><strong>Back-end Developer</strong></td>
    <td>9V</td>
  </tr>
</table>
</p>
